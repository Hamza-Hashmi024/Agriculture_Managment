...
const handleExtendSubmit = async () => {
  ...
  onClick={() => {
    // CSV Headers
    const headers = ["Buyer Name", "Remaining Due", "Overdue Due", "Due Soon", "Next Due Date"];

    // CSV Rows
    const rows = receivables.map((buyer) => [
      buyer.buyerName,
      parseAmount(buyer.remainingDue),
      parseAmount(buyer.overdueDue),
      parseAmount(buyer.dueSoonDue),
      buyer.nextDueDate || "-",
    ]);
